...
def add_association_markers(fig, classes, subsets, scatter_x, scatter_y, max_y, params):
    ...
    for i, s in enumerate(subsets):
        ...
        for j in range(len(classes)):
            if classes[j] in s:
                ...
                marker_colors.append(params['category_color_mapping'][classes[j]])
                fig.add_trace(go.Scatter(
                    x=scatter_x_has,
                    y=scatter_y_has,
                    mode='markers+lines',
...
def add_category_labels(fig, classes, scatter_y, params):
    max_string_len = max([len(x) for x in classes])
    fig.add_trace(go.Scatter(
        x=[-0.01 * max_string_len]*len(classes), 
        y=scatter_y,
        text=[f'<b>{col}</b>' for col in classes],
        mode='text',
...
def add_category_counts_bar(fig, class_counts, params):
